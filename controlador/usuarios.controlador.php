  ...
  public static function ctrAgregarUsuarios()
  {
    if (isset($_POST['btnAgregarUsuario'])) {
      ...
      if ($guardarUsuario) {
        ...
        $usr = UsuariosModelo::mdlConsultarUsuarioID($_POST['usr_correo']);
        ...
        echo '<script>
                    swal({
                        title: "¡Muy bien!",
                        text: "Bienvenido a la aplicación ' . $_SESSION['session_usr']['usr_nombre'] . '",
                        icon: "success",
        ...
        echo
          '<br>
            <div class=" mt-4 alert alert-danger text-center" role="alert">
                Este usuario ya existe, intente con otro
            </div>
          ';
        return;
      }
      ...
      if ($actualizarUsuario) {
        ...
        echo '<script>
                swal({
                  ...
                  .then((willDelete) => {
                    if (willDelete) {
                      window.location.href = "' . HTTP_HOST . 'lista-usuarios"
                    }
                  });
                </script>';
      } else {
